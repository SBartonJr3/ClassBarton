...
unsigned long shadow_sequence_number(int placeNumber)
{
    ...
    if (placeNumber <= 0)
    {
        return 105;    //if the number entered is negative
    }
    else if(placeNumber > 2300)
    {
        return 105; // if the number is over 2300
    
    }
    else if (placeNumber == 1)
    {
        return 9; //the first number in shadow sequence is always 9
    }
    else if (placeNumber == 2)
    {
        return 0; //the second number in shadow sequence is always 0
    }
    else
    {    
        return (shadow_sequence_number(placeNumber -1) + (placeNumber * placeNumber)); //S(n) = S(n-1) + (n * n)
