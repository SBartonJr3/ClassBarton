 ...
 *             There is no guarantee sentence_ptr change main()an address (return NULL if it's NULL and assign the
 *                 proper error code)
...
char * find_the_word(char * sentence_ptr, char * searchWord_ptr, int * errorCode_ptr)
{
    if (sentence_ptr == NULL)  // if sentence is passed return NULL
    {
        
        *errorCode_ptr = ERROR_NULL_SENTENCE_POINTER; 
        return NULL;
    }
    else if (searchWord_ptr == NULL) //if searchword is passed return NULL
    {
        
        *errorCode_ptr = ERROR_NULL_SENTENCE_POINTER;
        return NULL;
    }
    else if (errorCode_ptr == NULL) //if errorcode is passed return NULL
    {
        return NULL;
    }
    	for(int i = 0; i < strlen(sentence_ptr); i++)           //For every letter in the sentence
    {
        if((strlen((sentence_ptr + i)) - strlen(searchWord_ptr)) >= 0)    //If our search word fits then search for it
        {
            for(int j = 0; j < strlen(searchWord_ptr); j++)         //For every letter in search word
            {
                ...
                else if(j == strlen(searchWord_ptr) - 1)    //if the value is equal to the last value then exit
                {
