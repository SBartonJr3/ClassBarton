...
def ChangeLetter(inputStr, badLtr, goodLtr):
    if not inputStr or not badLtr or not goodLtr:
...
def ObfuscateString(inputStr, key):
    ...
    for char in inputStr:
        tempStor.append(chr(ord(char)^key))
    return ''.join(tempStor)
...
def InsertWord(inputStr, word, target):
    if not inputStr or not word or not target:
        return ERROR_INVALID
    elif inputStr.find(target) == -1:
        return ERROR_NOT_FOUND
    else:
        length = len(target)
        location = inputStr.find(target)+length
        outputStr = inputStr[:location] +word +inputStr[location:]
        return outputStr
